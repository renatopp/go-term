package term

import (
	"strconv"
	"strings"
	"time"

	"github.com/renatopp/go-term/term/ui"
)

// DefaultFloatCounterSpeed is the time a FloatCounter takes to animate from
// its value at the moment WithValue is called to the new target, used until
// WithSpeed is called.
const DefaultFloatCounterSpeed = 500 * time.Millisecond

// DefaultFloatCounterFormat is the function used to render a FloatCounter's
// value until WithFormat is called: the value fixed to two decimal places.
var DefaultFloatCounterFormat = func(v float64) string { return strconv.FormatFloat(v, 'f', 2, 64) }

// FloatCounter renders a floating-point value that eases toward a target
// value over time instead of jumping to it immediately. WithValue sets a new
// target and lets it animate there, taking Speed to arrive; Jump sets a
// value immediately with no animation. It does not animate on its own; call
// Tick whenever a new frame should be shown (e.g. from a program-driven
// timer, see Program.WithTick).
type FloatCounter struct {
	value       float64
	target      float64
	startValue  float64
	speed       time.Duration
	elapsed     time.Duration
	lastTime    time.Time
	format      func(float64) string
	prefix      string
	prefixStyle *Style
	suffix      string
	suffixStyle *Style
	style       *Style
}

func NewFloatCounter(initial float64) *FloatCounter {
	return &FloatCounter{
		value:      initial,
		target:     initial,
		startValue: initial,
		speed:      DefaultFloatCounterSpeed,
		format:     DefaultFloatCounterFormat,
	}
}

// Value returns the counter's current, possibly mid-animation, value.
func (c *FloatCounter) Value() float64 {
	return c.value
}

// WithValue sets the value the counter animates toward, easing from its
// current value over the duration set by WithSpeed. Call Tick to advance the
// animation. Use Jump to change the value immediately instead.
func (c *FloatCounter) WithValue(v float64) *FloatCounter {
	if v == c.target {
		return c
	}
	c.startValue = c.value
	c.target = v
	c.elapsed = 0
	c.lastTime = time.Time{}
	return c
}

// Target returns the value the counter is currently animating toward.
func (c *FloatCounter) Target() float64 {
	return c.target
}

// Jump immediately sets both the current and target value, skipping the
// animation.
func (c *FloatCounter) Jump(v float64) *FloatCounter {
	c.value = v
	c.target = v
	c.startValue = v
	c.elapsed = 0
	c.lastTime = time.Time{}
	return c
}

// Animating reports whether the counter's value has not yet reached its
// target.
func (c *FloatCounter) Animating() bool {
	return c.value != c.target
}

func (c *FloatCounter) Speed() time.Duration {
	return c.speed
}

// WithSpeed sets how long the counter takes to animate from its value at the
// time of the change to its current target, restarting the animation from
// the current value.
func (c *FloatCounter) WithSpeed(d time.Duration) *FloatCounter {
	c.speed = d
	c.startValue = c.value
	c.elapsed = 0
	c.lastTime = time.Time{}
	return c
}

func (c *FloatCounter) Format() func(float64) string {
	return c.format
}

// WithFormat sets the function used to render the counter's current value,
// e.g. to change its decimal precision or add a unit suffix.
func (c *FloatCounter) WithFormat(f func(float64) string) *FloatCounter {
	c.format = f
	return c
}

func (c *FloatCounter) Prefix() string {
	return c.prefix
}

// WithPrefix sets the text rendered before the counter's value.
func (c *FloatCounter) WithPrefix(s string) *FloatCounter {
	c.prefix = s
	return c
}

func (c *FloatCounter) PrefixStyle() *Style {
	return c.prefixStyle
}

// WithPrefixStyle sets the style applied to the counter's prefix when
// rendering.
func (c *FloatCounter) WithPrefixStyle(s Style) *FloatCounter {
	c.prefixStyle = &s
	return c
}

// WithoutPrefixStyle removes the counter's prefix style, rendering it plain.
func (c *FloatCounter) WithoutPrefixStyle() *FloatCounter {
	c.prefixStyle = nil
	return c
}

func (c *FloatCounter) Suffix() string {
	return c.suffix
}

// WithSuffix sets the text rendered after the counter's value.
func (c *FloatCounter) WithSuffix(s string) *FloatCounter {
	c.suffix = s
	return c
}

func (c *FloatCounter) SuffixStyle() *Style {
	return c.suffixStyle
}

// WithSuffixStyle sets the style applied to the counter's suffix when
// rendering.
func (c *FloatCounter) WithSuffixStyle(s Style) *FloatCounter {
	c.suffixStyle = &s
	return c
}

// WithoutSuffixStyle removes the counter's suffix style, rendering it plain.
func (c *FloatCounter) WithoutSuffixStyle() *FloatCounter {
	c.suffixStyle = nil
	return c
}

func (c *FloatCounter) Style() *Style {
	return c.style
}

// WithStyle sets the style applied to the counter's value when rendering.
func (c *FloatCounter) WithStyle(s Style) *FloatCounter {
	c.style = &s
	return c
}

// WithoutStyle removes the counter's value style, rendering it plain.
func (c *FloatCounter) WithoutStyle() *FloatCounter {
	c.style = nil
	return c
}

// Tick advances the counter's value toward its target based on the time
// elapsed since the previous call, given the current time (typically
// TickEvent.Time). It has no effect once the value reaches the target, or
// while the speed is zero or negative.
func (c *FloatCounter) Tick(now time.Time) *FloatCounter {
	if c.value == c.target || c.speed <= 0 {
		return c
	}
	if c.lastTime.IsZero() {
		c.lastTime = now
		return c
	}

	c.elapsed += now.Sub(c.lastTime)
	c.lastTime = now

	if c.elapsed >= c.speed {
		c.value = c.target
		return c
	}

	frac := float64(c.elapsed) / float64(c.speed)
	c.value = c.startValue + (c.target-c.startValue)*frac
	return c
}

// PreferredWidth returns the widest of the counter's current and target
// values (its formatted text's two endpoints) plus its prefix and suffix,
// so its size doesn't jitter as it animates.
func (c *FloatCounter) PreferredWidth() int {
	w := max(ui.StringWidth(c.format(c.value)), ui.StringWidth(c.format(c.target)))
	return ui.StringWidth(c.prefix) + w + ui.StringWidth(c.suffix)
}

func (c *FloatCounter) PreferredHeight(width int) int {
	return 1
}

// Update advances the counter's animation on every TickEvent (see
// Program.WithTick).
func (c *FloatCounter) Update(e Event) Event {
	if t, ok := e.(TickEvent); ok {
		c.Tick(t.Time)
	}
	return e
}

// Render draws the counter's prefix, current value, and suffix — each styled
// independently — clipped to width columns.
func (c *FloatCounter) Render(width, height int) []string {
	if width <= 0 || height <= 0 {
		return nil
	}

	var b strings.Builder
	remaining := width
	remaining = renderCounterSegment(&b, c.prefix, c.prefixStyle, remaining)
	remaining = renderCounterSegment(&b, c.format(c.value), c.style, remaining)
	renderCounterSegment(&b, c.suffix, c.suffixStyle, remaining)

	return []string{b.String()}
}
